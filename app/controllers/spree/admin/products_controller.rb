  ...
  module Admin
    class ProductsController < ResourceController
      ...
      def update
        ...
        # name = params[:product][:certificate].original_filename
        # directory = "public/files"
        # path = File.join(directory, name)
        # File.open(path, "wb") { |f| f.write(params[:product][:certificate].read) }
        #
        ...
        if @object.update_attributes(permitted_resource_params)
          unless cert.blank?
            name = cert.original_filename
            directory = "public/certificates"
            path = File.join(directory, name)
            File.open(path, "wb") { |f| f.write(cert.read) }
            @object.certificate = '/certificates/'+path.split('/').last
            @object.save!
          end
          unless instruction.blank?
            name = instruction.original_filename
            directory = "public/instructions"
            path = File.join(directory, name)
            File.open(path, "wb") { |f| f.write(instruction.read) }
